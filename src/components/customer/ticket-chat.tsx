...
} from "@/hooks/customer/useCreateTicket";
import { useGetTickets } from "@/hooks/customer/useGetTickets";
import {
  useGetTicketDetails,
  type TicketDetails,
} from "@/hooks/customer/useGetTicketDetails";
import {
  useSendTicketMessage,
  type SendMessageData,
} from "@/hooks/customer/useSendTicketMessage";
import {
  useCreateBooking,
  type CreateBookingRequest,
  type CreateBookingResponse,
} from "@/hooks/customer/useCreateBooking";
import {
  Paperclip,
  Send,
  User,
...
export function TicketChat({
  ...
  const [formData, setFormData] = useState<Partial<CreateTicketData>>({
    ...
    description: serviceInfo
      ? `Hi! I'm interested in your service "${serviceInfo.title}". Could you please provide more information?`
      : "",
  ...
  const { data: ticketDetailsResponse } = useGetTicketDetails(
    selectedTicket?.ticket_id || null
  );
  ...
  useEffect(() => {
    if (ticketDetailsResponse?.data) {
      ...
      const apiMessages: Message[] = ticketDetailsResponse.data.messages.map(
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      const bookingPayload: CreateBookingRequest = {
        ...
        address:
          bookingData.delivery_method === "onsite" ||
          bookingData.delivery_method === "pickup" ||
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      toast.error(
        `Sorry, there was an issue creating your ticket: ${errorMessage}. Please try again.`
      );
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const serviceId =
        serviceInfo?.id ||
        selectedTicketDetails?.service_detail.id.toString() ||
        "13";
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "ongoing":
        return "bg-[#F28C0D]/10 text-[#F28C0D] border-[#F28C0D]/20";
      case "close":
        return "bg-green-50 text-green-700 border-green-200";
      default:
        return "bg-gray-50 text-gray-700 border-gray-200";
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <div
                              className={`w-2 h-2 rounded-full ${ticket.online_status === "online" ? "bg-green-500" : "bg-gray-400"}`}
                            ></div>
                          </div>
                        </div>
                ...
                <div className="flex items-center space-x-3">
                  {serviceInfo?.shop?.logo || selectedTicket.profile_photo ? (
                    <div className="w-10 h-10 rounded-full overflow-hidden">
                      <img
                        src={
                          serviceInfo?.shop?.logo ||
                          selectedTicket.profile_photo
                        }
                        alt={
                          serviceInfo?.shop?.name || selectedTicket.full_name
                        }
                        className="w-full h-full object-cover"
                      />
                    </div>
                    ...
                    <h3 className="font-semibold text-gray-900">
                      {serviceInfo
                        ...
                        : selectedTicketDetails
                          ? selectedTicketDetails.service_detail.name
                          : selectedTicket.full_name}
                    </h3>
                    <div className="text-sm text-green-500 flex items-center">
                      <div
                        className={`w-2 h-2 rounded-full mr-2 ${serviceInfo || (selectedTicketDetails ? selectedTicketDetails.user_detail.online_status === "online" : selectedTicket.online_status === "online") ? "bg-green-500" : "bg-gray-400"}`}
                      ></div>
                      {serviceInfo
                        ? "Service Provider"
                        : selectedTicketDetails
                          ? selectedTicketDetails.user_detail.online_status ===
                            "online"
                            ? "Online"
                            : "Offline"
                          : selectedTicket.online_status === "online"
                            ? "Online"
                            : "Offline"}
                    </div>
                  </div>
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto bg-gray-50/30">
              <div className="text-center">
                <span className="text-xs text-gray-500 bg-white px-3 py-1 rounded-full border border-gray-200">
                  {new Date(
                    selectedTicket.last_message_time
                ...
                <div className="flex justify-center">
                  <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 max-w-sm">
                    <div className="text-center">
                      ...
                      <div className="text-[#F28C0D] font-semibold text-lg mb-2">
                        {serviceInfo.title}
                      </div>
                      <div className="text-xs text-gray-500">
                        You're now connected with {serviceInfo.shop.name}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex-1 max-w-md">
                    ...
                    >
                      ...
                      {msg.sender === "user" && (
                        <svg
                          className="w-4 h-4 text-green-500 inline ml-2"
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                          <path
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              ...
              {attachedFile && (
                <div className="flex items-center justify-between bg-[#F28C0D]/10 border border-[#F28C0D]/20 p-3 rounded-xl mt-3">
                  <span className="text-sm text-[#F28C0D] font-medium">
                    {attachedFile.name}
                  </span>
                  <button
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50/30">
            <div className="text-center text-gray-500 max-w-sm">
              ...
              <p className="text-gray-500 mb-6">
                Select a ticket to view the conversation or create a new support
                ticket to get help.
              </p>
              <button
                onClick={() => setShowCreateForm(true)}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setBookingData((prev) => ({
                            ...prev,
                            delivery_method: e.target.value as
                              | "online"
                              | "virtual"
                              | "remote"
                              | "onsite"
