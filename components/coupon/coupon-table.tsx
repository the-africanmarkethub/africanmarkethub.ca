...
export function CouponTable() {
  ...
  const [couponToDelete, setCouponToDelete] = React.useState<CouponTableItem | null>(null);
  ...
  const coupons: CouponTableItem[] = couponsResponse?.data?.map(transformCouponToTableItem) || [];
  ...
  const handleEditCoupon = (coupon: CouponTableItem) => {
    ...
    const fullCoupon = couponsResponse?.data?.find((c: { id: number }) => c.id === coupon.id);
    
    if (fullCoupon) {
      // Encode the coupon data and pass as URL parameter
      const encodedData = encodeURIComponent(JSON.stringify(fullCoupon));
      router.push(`/products/promotion/edit-coupon/${coupon.id}?data=${encodedData}`);
    }
  };
        ...
        {isLoading ? (
          ...
          <div className="w-full space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button
                onClick={() => router.push("/products/promotion/add-coupon")}
