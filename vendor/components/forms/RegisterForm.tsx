...
import { BusinessTypes, FormFieldType } from "@/constants/formFieldType";
...
const RegisterForm = () => {
  ...
  const subscriptionOptions =
    subcription?.map((item: {id: number; name: string; monthly_price: number}) => ({
  ...
  useEffect(() => {
    ...
    if (location && location?.length > 0) {
      const countries = location?.map((country: {id: number; name: string; state: Array<{id: number; name: string}>; city: Array<{id: number; name: string; state_id: number}>}) => ({
        label: country.name,
  ...
  useEffect(() => {
    if (country_id && location) {
      const selectedCountry = location.find(
        (country: {id: number; name: string; state: Array<{id: number; name: string}>; city: Array<{id: number; name: string; state_id: number}>}) => country.id.toString() === country_id
      );

      if (selectedCountry && selectedCountry.state) {
        const states = selectedCountry.state.map((state: {id: number; name: string}) => ({
          label: state.name,
    ...
    if (state_id && country_id && location) {
      ...
      if (selectedCountry) {
        const cities = selectedCountry.city
          .filter((city: {state_id: number; name: string; id: number}) => city.state_id.toString() === state_id)
  ...
  const handleNext = async () => {
    ...
    const result = await form.trigger(step1Fields as (keyof z.infer<typeof VendorFormValidation>)[]);
  ...
  async function onSubmit(values: z.infer<typeof VendorFormValidation>) {
    ...
    const payload = {
      ...
      type: values.type === "products" ? "product" : "service" as "product" | "service",
